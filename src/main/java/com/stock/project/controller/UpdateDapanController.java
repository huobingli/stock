...
//            JSONObject json = new JSONObject(string);
            ...
            List<DapanLimitUpDownEntity> arrayList;
            for (int i = 0; i < arrayData.size(); i++) {
                JSONObject data = arrayData.getJSONObject(i);

                DapanLimitUpDownEntity dp;
                dp.setId(data.get("id")).setTime(data.get("ctime")).setLimitdown(data.get("limitdown")).setLimitup(data.get("limitup"));
                arrayList.add(dp);
            }
        } catch (Exception e)
        {
